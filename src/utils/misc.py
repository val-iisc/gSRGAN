...
def check_flags(train_configs, model_configs, n_gpus):
    ...
    elif model_configs['data_processing']['dataset_name'] in ['imagenet', 'tiny_imagenet']:
...
def plot_sn_curves(gain_sn, name, run_name, step_count):
        ''' gain_sn : [ LAYERS x CLASSES x 256 ] 
        '''
        directory = join('./figures', f"{run_name}/Param_Plots_{step_count}")

        if not exists(abspath(directory)):
            os.makedirs(directory)

        # save_path = join(directory, f'_Corr_{name}.png')

        for l in range(0, len(gain_sn), 2):
            for c in range(0, gain_sn[0].shape[0], 3):

                save_path = join(directory , f"Layer_{l+1}_Class_{c+1}__Corr_{name}.png")
                vec = gain_sn[l][c, :]
                a, b = get_dimensions(vec.shape[0])
                vec = vec.reshape(a,b).double()
                # tmp = vec.cpu().detach().numpy().reshape(-1,1)
                tmp = vec.cpu().detach().numpy()
                
                tmp_mean_centered = tmp-tmp.mean()

                row_norms = np.linalg.norm(tmp_mean_centered, axis=1)
                tmp_mean_centered = tmp_mean_centered / row_norms[:, None]  # Normalize reshaped embedding vector_to_matrix on rows

                cov = (1./len(tmp)) * np.dot((tmp_mean_centered), (tmp_mean_centered).transpose())

                plt.imshow(cov, cmap='hot', interpolation='nearest', vmin=-0.06, vmax=0.06)
                plt.colorbar()
                plt.savefig(save_path)
                plt.close()

def plot_attention_maps(generated_images, grayscale_cams, run_name, img_name, classes_type, ncol, logger, heatmaps = False, logging=True):
    from pytorch_grad_cam.utils.image import show_cam_on_image

    imgs = []
    for i, img in enumerate(generated_images):
        if heatmaps:
            ### show_cam_on_image function returns a (0,255) image
            visualization = show_cam_on_image(img.permute(1,2,0).detach().cpu().numpy(), np.float32(grayscale_cams[i]), use_rgb=True)
        else:
            if img.max() > 1:
                raise Exception(
                    "The input image should np.float32 in the range [0, 1]")
            cam = img.permute(1,2,0).detach().cpu().numpy() + np.expand_dims(grayscale_cams[i], axis=2)
            cam = cam / np.max(cam)
            visualization = np.uint8(255 * cam)
        imgs.append(visualization/255)

    visualization = np.float32(imgs)
    visualization = torch.tensor(visualization).permute(0,3,1,2)
    plot_img_canvas(visualization, "./figures/{run_name}/{classes_type}/{img_name}.png".\
                    format(run_name=run_name, classes_type=classes_type, img_name=img_name), ncol, logger, logging=logging)
...
def plot_tsne_scatter_plot(df, tsne_results, flag, run_name, logger, logging=True, cfgs=None):
    if logger is None: logging=False
    ...
    sns.scatterplot(
        ...
        hue="labels",
        palette=sns.color_palette("hls", cfgs.num_classes),
        data=df,
        legend="full",
        alpha=0.5
...
def plot_corr_matrix(corr_matrix, img_name, classes_type, labels, run_name, logger, logging=True):
    if logger is None: logging=False
    directory = join('./figures', run_name, classes_type)

    if not exists(abspath(directory)):
        os.makedirs(directory)

    save_path = join(directory, f"{img_name}.png")
    ...
    sns.heatmap(corr_matrix, cmap= "inferno",
            ...
            square=True, linewidths=.002,
            annot=False, cbar_kws={"shrink": 0.8}, vmax=1e-4, vmin=-1e-4)

    ax.set_title("Heatmap of correalation scores").set_fontsize(15)
    ax.set_xlabel("")
    ax.set_ylabel("")
