...
def main():
    ...
    parser.add_argument('-c', '--config_path', type=str, default='./src/configs/CIFAR10/SNGAN.json')
    ...
    parser.add_argument('--seed', type=int, default=42, help='seed for generating random numbers')
    ...
    parser.add_argument('-eb', '--eval_backbone', type=str, default='Inception_V3')
    ...
    parser.add_argument('-tsne_lt', '--tsne_analysis_lt', action='store_true', help='whether conduct tsne analysis on long tail dataset')
    parser.add_argument('-cr_mat', '--corr_mat', action='store_true', help='whether create the correlation matrix')
    parser.add_argument('-att_maps', '--attention_maps', action='store_true', help='whether visualize the attention maps')
    ...
    if not args.train and \
            ...
            not args.tsne_analysis_lt and \
            not args.corr_mat and \
            not args.attention_maps:
    ...
    hdf5_path_train = make_hdf5(model_configs['data_processing'], train_configs, "train", model_configs) \
        if train_configs['load_all_data_in_memory'] else None
    
    make_hdf5(model_configs['data_processing'], train_configs, "val", model_configs) \
        if train_configs['load_all_data_in_memory'] else None
    ...
    if train_configs['seed'] == -1:
        ...
        cudnn.benchmark, cudnn.deterministic = True, True # For faster processing on RTX 3090
