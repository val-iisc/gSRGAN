...
def generate_images_and_stack_features(generator, discriminator, eval_model, num_generate, batch_size, z_prior,
                                       truncation_factor, num_classes, world_size, DDP, device, logger, disable_tqdm, latent_op, latent_op_step, latent_op_alpha, latent_op_beta):
    eval_model.eval()
    ...
    for i in tqdm(range(num_batches), disable=disable_tqdm):
        # batch_size, gen, dis, truncated_factor, prior, latent_op, latent_op_step,
        #             latent_op_alpha, latent_op_beta, device
        fake_images, fake_labels = generate_images(
                                                    batch_size=batch_size,
                                                    truncated_factor = truncation_factor,
                                                    prior = z_prior,
                                                    gen=generator,
                                                    dis=discriminator,
                                                    latent_op = latent_op,
                                                    latent_op_step = latent_op_step,
                                                    latent_op_alpha = latent_op_alpha,
                                                    latent_op_beta = latent_op_beta,
                                                    device=device,
                                                   )
        with torch.no_grad():
            features, logits = eval_model.get_outputs(fake_images)
...
def sample_images_from_loader_and_stack_features(dataloader, eval_model, batch_size, world_size,
    ...
    for i in tqdm(range(0, num_batches), disable=disable_tqdm):
        ...
        with torch.no_grad():
            features, logits = eval_model.get_outputs(images)
            probs = torch.nn.functional.softmax(logits, dim=1)
