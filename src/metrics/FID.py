...
def get_activations(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor, prior, is_generate,
                    latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, tqdm_disable=False, run_name=None, class_dist=None, sampler=None):
    ...
    if is_generate is True:
        ...
        if data_loader.sampler is None or isinstance(data_loader.sampler, torch.utils.data.sampler.SequentialSampler):
    ...
    img_arr = np.empty((total_instance, 3, generator.img_size, generator.img_size)) #modularize
...
def calculate_activation_statistics(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor, prior,
    ...
    act, img_arr = get_activations(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor, prior,
                          is_generate, latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, tqdm_disable, run_name)
    # print("activations: ",act, act.shape)
    mu = np.mean(act, axis=0)
    sigma = np.cov(act, rowvar=False)
    # print("Sigma: ",sigma, sigma.shape)
    return mu, sigma, act, img_arr
...
def calculate_fid_score(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor, prior,
    ...
    if pre_cal_mean is not None and pre_cal_std is not None:
        ...
        m1, s1, _, _ = calculate_activation_statistics(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor,
                                                 prior, False, False, 0, latent_op_alpha, latent_op_beta, device, tqdm_disable=disable_tqdm)

    m2, s2, _, _ = calculate_activation_statistics(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor, prior,
...
def mahalanobis_dist(x, mean, inv_cov):
    a = (x - mean) @ inv_cov 
    return np.sqrt(np.diag(a @ ((x - mean).T)))


def get_bottom(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor, prior,
                        latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, logger, pre_cal_mean=None, pre_cal_std=None, run_name=None):
    disable_tqdm = device != 0
    inception_model.eval()

    if device == 0: logger.info("Getting Bottom MDIST Images....")
    mean, cov, act, img_arr = calculate_activation_statistics(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor, prior,
                                             True, latent_op, latent_op_step, latent_op_alpha, latent_op_beta, device, tqdm_disable=disable_tqdm, run_name=run_name)

    if pre_cal_mean is not None and pre_cal_std is not None:
        mean_real, cov_real = pre_cal_mean, pre_cal_std
    else:
        mean_real, cov_real, _, _ = calculate_activation_statistics(data_loader, generator, discriminator, inception_model, n_generate, truncated_factor,
                                                 prior, False, False, 0, latent_op_alpha, latent_op_beta, device, tqdm_disable=disable_tqdm)


    inv_cov = np.linalg.inv(cov_real)
    mdists = []
    mdists.append(mahalanobis_dist(act, mean_real, inv_cov))
    mdists = np.concatenate(mdists)    
    idxs = np.argsort(mdists)
    print(mdists[idxs[0]], mdists[idxs[-1]])
    print(mdists.min(), mdists.max())
    bottom = img_arr[-1: -31: -1]
