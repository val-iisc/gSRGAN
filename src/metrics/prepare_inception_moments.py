...
def prepare_inception_moments(dataloader, eval_mode, generator, inception_model, splits, run_name, logger, device):
    ...
    if isinstance(dataloader.dataset, LoadDataset):
        img_size = dataloader.dataset.resize_size
    else:
        img_size = 32
    
    inception_model.eval()
    net_name = 'inception'
    if isinstance(inception_model, object):
        net_name = inception_model.eval_backbone.lower().split('_')[0]
    save_path = os.path.abspath(os.path.join("./data", dataset_name + "_" + str(img_size) + "_" + eval_mode +'_' + net_name + '_moments.npz'))
    ...
    if is_file:
        ...
        mu, sigma, _, _ = calculate_activation_statistics(data_loader=dataloader,
                                                    generator=generator,
                                                    discriminator=None,
